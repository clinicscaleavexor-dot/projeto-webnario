-- =====================================================================
--  Atualiza get_pending_reminders: pós-aula passa a usar janela
--  window_start_minutes → window_end_minutes (igual ao pré-aula).
--
--  Também insere chaves de fallback global para a janela pós-aula
--  em dispatch_settings.
--
--  Rode no Supabase: SQL Editor -> New query -> Run.
-- =====================================================================

-- Insere defaults da janela pós-aula no fallback global
INSERT INTO public.dispatch_settings (key, value)
VALUES
  ('pos_window_start_minutes', '60'),
  ('pos_window_end_minutes',   '90')
ON CONFLICT (key) DO NOTHING;

-- Recria a função com lógica de janela para pós-aula
...
BEGIN
  ...
  RETURN QUERY
    ...
    WHERE EXISTS (
      ...
      AND (
        CASE
          -- Janela configurada por webinário
          WHEN (w.settings->'dispatch_config'->'pos'->>'enabled') = 'true' THEN
            sl.scheduled_for >= now() - COALESCE(
              (w.settings->'dispatch_config'->'pos'->>'window_end_minutes')::int, 90
            ) * interval '1 minute'
            AND sl.scheduled_for <= now() - COALESCE(
              (w.settings->'dispatch_config'->'pos'->>'window_start_minutes')::int, 60
            ) * interval '1 minute'
          -- Sem config: usa janela global (p_pos_min/max)
          WHEN w.settings->'dispatch_config' IS NULL THEN
